...
typedef enum { BAT_NCH, BAT_DSC, BAT_CHR, BAT_FULL } BatStatus;
...
int __get_bat_perc(const char *bat, PercInfo *bat_info, PercType mode) {
    char path[PATH_MAX];

    if (esnprintf(path, sizeof(path), POWER_SUPPLY_CAPACITY, bat) < 0)
        return -1;
    if (pscanf(path, "%d", &bat_info->perc) != 1)
        return -1;

    bat_info->perc_repr = bprintf("%d", bat_info->perc);
...
const char *battery_perc(const char *bat) {
    PercInfo bat_info = {0};
    int s = __get_bat_perc(bat, &bat_info, PERC_STR);

    if (s == -1)
        return NULL;

    return bat_info.perc_repr;
}

const char *__process_bat_info(const char *state, PercInfo *perc) {
    static struct {
        const char *state;
        BatStatus status;
    } map[] = {{"Full", BAT_FULL},
...
const char *battery_state(const char *bat) {
    ...
    PercInfo bat_info = {0};
    int s = __get_bat_perc(bat, &bat_info, PERC_INT);

    if (s == -1)
        return "?";

    const char *symbol = __process_bat_info(state, &bat_info);
